...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <h5 class="card-header">Update Timeline</h5>
                    <div class="card-body">
                        <form action="{{route('timeline.update.data', $timeline->id)}}" method="POST">
                            @csrf
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select name="user_id" id="user_id" class="form-select">
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ old('user_id', $timeline->user_id == $user->id) ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-6">
                                    ...
                                    {!! colorDropDown($colors, old('color_id', $timeline->color_id)) !!}
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <input type="text" class="form-control" name="name" value="{{old('name', $timeline->name)}}" placeholder="Enter name"/>
                                </div>
                                <div class="col-md-6 mb-6">
                                    <label class="form-label">Description
                                        <i class="fa fa-asterisk small-font text-danger" aria-hidden="true"></i>
                                    </label>
                                    <textarea class="form-control" name="description" placeholder="Enter description" cols="3" rows="3">{{ old('description', $timeline->description) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="1" @selected(old('status', $timeline->status) == 1)>Active</option>
                                        <option value="0" @selected(old('status', $timeline->status) == 0)>Inactive</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row mt-3">
