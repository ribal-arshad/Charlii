...
@section('title', 'Update Timeline Event Type')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <h5 class="card-header">Update Timeline Event Type</h5>
                    <div class="card-body">
                        <form action="{{route('timeline.event.type.update.data', $eventType->id)}}" method="POST">
                            @csrf
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select name="user_id" id="user_id" class="form-select">
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ old('user_id', $eventType->user_id == $user->id) ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-6">
                                    ...
                                    {!! colorDropDown($colors, old('color_id', $eventType->color_id)) !!}
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select class="form-control" name="timeline_id" id="timelines">
                                        @foreach ($timelines as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $item->id, $selectedTimeline ? 'selected' : '' }}>
                                                {{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-6">
                                    ...
                                    <input type="text" class="form-control" name="event_type" value="{{old('event_type', $eventType->event_type)}}" placeholder="Enter chapter name"/>
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="1" @selected(old('status', $eventType->status) == 1)>Active</option>
                                        <option value="0" @selected(old('status', $eventType->status) == 0)>Inactive</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row mt-3">
