...
@section('title', 'Update Plot Planner')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <h5 class="card-header">Update Plot Planner</h5>
                    <div class="card-body">
                        <form action="{{route('planner.update.data', $planner->id)}}" method="POST">
                            @csrf
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select name="user_id" id="user_id" class="form-select">
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ old('user_id', $planner->user_id == $user->id) ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-6">
                                    ...
                                    {!! colorDropDown($colors, old('color_id', $planner->color_id)) !!}
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    <label class="form-label">Plot Planner Title
                                        <i class="fa fa-asterisk small-font text-danger" aria-hidden="true"></i>
                                    </label>
                                    <input type="text" class="form-control" name="plot_planner_title" value="{{old('plot_planner_title', $planner->plot_planner_title)}}" placeholder="Enter plot planner title"/>
                                </div>
                                <div class="col-md-6 mb-6">
                                    <label class="form-label">Plot Planner Description
                                        <i class="fa fa-asterisk small-font text-danger" aria-hidden="true"></i>
                                    </label>
                                    <textarea class="form-control" name="description" placeholder="Enter description" cols="3" rows="3">{{ old('description', $planner->description) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 mb-6">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="1" @selected(old('status', $planner->status) == 1)>Active</option>
                                        <option value="0" @selected(old('status', $planner->status) == 0)>Inactive</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row mt-3">
